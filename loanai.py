...
# 3. Bulletproof PDF Generator (Using fpdf2 syntax)
def generate_pdf(data_dict):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("helvetica", "B", 16)
    
    # Header - No special symbols
    pdf.cell(0, 10, text="LOAN ASSESSMENT OFFICIAL REPORT", ln=True, align='C')
    pdf.ln(10)
    
    pdf.set_font("helvetica", size=12)
    for key, value in data_dict.items():
        # Strip everything except basic alphanumeric for maximum safety
        clean_line = f"{key}: {value}".encode('ascii', 'ignore').decode('ascii')
        pdf.cell(0, 10, text=clean_line, ln=True)
    
    pdf.ln(10)
    pdf.set_font("helvetica", "I", 10)
    pdf.cell(0, 10, text="Generated by AI Credit Risk System. Bank Confidential.", ln=True)
...
if submit:
    if model is None:
        st.error("Model file 'loan_models.pkl' not found! Check your folder.")
    else:
        # 1. Financial Math
        ...
        if dti_ratio > 0.60 or credit_score < 450:
            ...
            st.warning(f"High DTI Ratio ({dti_ratio:.1%}) or Low FICO Score ({credit_score}).")
        else:
            # 3. AI Prediction
            ...
            if risk_prob > 35:
                ...
                st.info("The application has been flagged for high probability of default.")
            else:
                ...
                # 5. Data for PDF (Cleaned)
                report_data = {
                    "Decision": "APPROVED",
                    "Income": f"{income}",
                    "Loan Amount": f"{loan_amt}",
                    "FICO Score": f"{credit_score}",
                    "Risk Prob": f"{risk_prob:.1f} percent",
                    "Monthly EMI": f"{emi:.2f}",
                    "DTI Ratio": f"{dti_ratio:.1%}"
                }
                
                try:
                    pdf_output = generate_pdf(report_data)
                    st.download_button(
                        label="📥 Download Approval Report",
                        data=pdf_output,
                        file_name=f"Report_{credit_score}.pdf",
